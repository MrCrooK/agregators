...
if($_REQUEST["numid"]) {
$query = "SELECT * FROM `lnmoney` WHERE `numid` = '".$_REQUEST["numid"]."' ORDER BY id DESC LIMIT 1"; //lnmoney
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
    <tbody><tr valign="top">
		...
		if(!$line["vk"]) echo "<div class='clOK'>Соц. сеть ОК данные отсутствуют</div>";

		if($line["vkvalid"]) echo $line["vk"]; 
		if(!$line["ok"]) echo "<div class='clVK'>Соц. сеть ВК данные отсутствуют</div>";
...
$query = "SELECT * FROM `lnmoney` ORDER BY id DESC"; //lnmoney
$result = mysql_query($query); 
