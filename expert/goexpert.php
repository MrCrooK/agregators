...
header("Content-Type: text/html; charset=cp1251");
	$fname	=	iconv('utf-8','cp1251',$_REQUEST["fname"]);
	$name	=	iconv('utf-8','cp1251',$_REQUEST["name"]);
	$oname	=	iconv('utf-8','cp1251',$_REQUEST["oname"]);
	$bdate	=	iconv('utf-8','cp1251',$_REQUEST["bdate"]);
	...
	if(($fname&&$name&&$oname)||($fname&&$name&&$bdate)) {
		include "query.php";
	} else echo "Отсутствуют ФИО или дата рождения.";
	} else {
	echo '<pre>';
